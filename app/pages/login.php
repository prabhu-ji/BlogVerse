  ...
  if(!empty($_POST))
  {
    ...
    if($row)
    {
      ...
      if(password_verify($_POST['password'], $row[0]['password']))
      {
        ...
        $role = $row[0]['role']; // Assuming 'role' is a field in the database

        if ($role === 'admin') {
            // Redirect to the admin dashboard
            redirect('admin');
        } elseif ($role === 'user') {
            // Redirect to the user dashboard/home page
            redirect('home'); // Replace 'home' with the actual route for user dashboard if needed
        } else {
            // Handle unknown roles or errors
            echo "<script>alert('Unknown role or unauthorized access.');</script>";
            redirect('login'); // Redirect to login or another appropriate page
        }
